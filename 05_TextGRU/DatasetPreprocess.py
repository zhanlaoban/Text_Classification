...
def GetIterator(TEXT, LABEL, path, args, **kwargs):
    ...
    train_iter, dev_iter, test_iter = data.Iterator.splits((train_dataset, dev_dataset, test_dataset), batch_sizes = (args.batch_size, len(dev_dataset)/8, len(test_dataset)/8), sort_key = lambda x: len(x.text), **kwargs)
