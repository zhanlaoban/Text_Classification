...
def train(train_iter, dev_iter, model, args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_iter:
            ...
            #input: [sentence_size, batchsize]
            #target: [batchsize]
            
            input = input.t()   #矩阵转置操作
            #input: [batchsize, sentence_size]
            ...
            #在每个tensor后面多了:device='cuda:0'
            
            optimizer.zero_grad()
            output = model(input)
            #output: [batchsize, classNum]

            loss = F.cross_entropy(output, target)
            #F.cross_entropy要求类的范围为[0, classnum-1]
            #loss: tensor(1.6840, device='cuda:0', grad_fn=<NllLossBackward>)
            loss.backward()
            #loss: tensor(1.6840, device='cuda:0', grad_fn=<NllLossBackward>)
...
def eval(data_iter, model, args):
    ...
    for batch in data_iter:
        ...
        #output: [batchsize, classNum]
        loss = F.cross_entropy(output, target)
        avg_loss += loss.item() #将loss累积
        corrects += (torch.max(output, 1)[1] == target).sum()   #将corrects累积

    
    evalSetSize = len(data_iter.dataset)    #dev set为50条, 所以值为50
